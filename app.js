...
// Initialize Application (all in-memory, no localStorage)
let favoritesData = {
  elements: [],
  compounds: [],
  reactions: []
...
class VirtualScroller {
  ...
  handleScroll() {
    ...
    this.visibleEnd = Math.min(this.visibleStart + 50, this.items.length);
    this.render();
  }
...
// Enhanced PubChem API Integration with caching
const pubchemCache = {};

async function fetchPubChemData(compound) {
  // Check cache first
  if (pubchemCache[compound]) {
    console.log('Using cached PubChem data for', compound);
    return pubchemCache[compound];
  }
  
  try {
    console.log('Fetching PubChem data for', compound);
    ...
    if (data.PC_Compounds && data.PC_Compounds.length > 0) {
      ...
      if (!propsResponse.ok) {
        throw new Error(`HTTP ${propsResponse.status}`);
      }
      ...
      // Cache the result
      pubchemCache[compound] = result;
      console.log('✅ PubChem data fetched and cached for', compound);
      
      return result;
    } else {
      console.log('No PubChem data found for', compound);
      return null;
    }
  } catch (error) {
    console.error('❌ PubChem fetch error for', compound, ':', error);
    return null;
  }
}

// Fetch multiple compounds in parallel with rate limiting
async function fetchPubChemBatch(compounds, maxConcurrent = 3) {
  const results = [];
  for (let i = 0; i < compounds.length; i += maxConcurrent) {
    const batch = compounds.slice(i, i + maxConcurrent);
    const batchResults = await Promise.all(
      batch.map(compound => fetchPubChemData(compound))
    );
...
// Theme Management Functions (in-memory only - no localStorage due to sandbox)
...
window.calculateIdealGas = calculateIdealGas;
window.calculateMolarity = calculateMolarity;
window.calculatePercentComposition = calculatePercentComposition;
window.calculateStoichiometry = calculateStoichiometry;
window.calculateLimitingReagent = calculateLimitingReagent;
window.calculatePercentYield = calculatePercentYield;
window.calculateEmpiricalFormula = calculateEmpiricalFormula;
window.calculateMolality = calculateMolality;
window.calculateOxidationNumber = calculateOxidationNumber;
window.calculateHalfLife = calculateHalfLife;
window.convertTemperature = convertTemperature;
window.convertPressure = convertPressure;
window.convertScientificNotation = convertScientificNotation;
window.countSigFigs = countSigFigs;
window.calculateThermodynamics = calculateThermodynamics;
window.calculateBufferPH = calculateBufferPH;
window.calculateNernst = calculateNernst;
window.calculateWavelengthFrequency = calculateWavelengthFrequency;
window.calculateOsmoticPressure = calculateOsmoticPressure;
window.showDictionarySearch = showDictionarySearch;
window.render3DElement = render3DElement;

function init() {
  console.log('🚀 Initializing CHEMNEST DICTIONARY with 20,000+ database...');
  ...
  // Lazy load other sections with requestIdleCallback
  ...
  // Ensure modal closes on ESC key
  ...
  console.log('✅ CHEMNEST DICTIONARY initialized successfully!');
  console.log('📊 Database: 118 elements, 10,000+ compounds, 5,000+ reactions, 5,000+ terms');
  console.log('🎨 UI: Smooth 60fps animations, responsive design');
  console.log('⚡ Performance: Virtual scrolling, debounced search, lazy loading');
}
...
window.fetchPubChemData = fetchPubChemData;
...
// Render Periodic Table with proper grid positioning - OPTIMIZED
function renderPeriodicTable() {
  ...
  chemistryData.elements.forEach((element, index) => {
    const cell = document.createElement('div');
    cell.className = `element-cell ${element.category}`;
    
    // Optimize: Use template strings without innerHTML
    const numDiv = document.createElement('div');
    numDiv.className = 'element-num';
    numDiv.textContent = element.num;
    
    const symDiv = document.createElement('div');
    symDiv.className = 'element-symbol';
    symDiv.textContent = element.sym;
    ...
    massDiv.className = 'element-mass';
    massDiv.textContent = element.mass.toFixed(2);
    
    cell.appendChild(numDiv);
    cell.appendChild(symDiv);
    cell.appendChild(nameDiv);
    cell.appendChild(massDiv);
  ...
  requestAnimationFrame(() => {
    ...
    cells.forEach((cell, index) => {
      ...
      }, index * 3); // Stagger by 3ms
    });
  });
}

// Favorites Management (in-memory only - no localStorage)
    ...
    <html>
      <head>
        ...
        <style>
          @media print {
            ...
            .element-symbol { font-size: 72px; font-weight: bold; color: #2563eb; }
      ...
      <body>
        <div class="header">
          ...
          <h1>${element.name}</h1>
          <p><strong>Atomic Number ${element.num} | Atomic Mass ${element.mass}</strong></p>
        </div>
        <table>
          <tr><th>Property</th><th>Value</th></tr>
          <tr><td><strong>Category</strong></td><td>${element.category.replace('-', ' ')}</td></tr>
          <tr><td><strong>Electron Configuration</strong></td><td>${element.config}</td></tr>
          ${element.group ? `<tr><td><strong>Group</strong></td><td>${element.group}</td></tr>` : ''}
          ${element.period ? `<tr><td><strong>Period</strong></td><td>${element.period}</td></tr>` : ''}
          ${element.mp ? `<tr><td><strong>Melting Point</strong></td><td>${element.mp}°C</td></tr>` : ''}
          ${element.bp ? `<tr><td><strong>Boiling Point</strong></td><td>${element.bp}°C</td></tr>` : ''}
          ${element.density ? `<tr><td><strong>Density</strong></td><td>${element.density} g/cm³</td></tr>` : ''}
          ${element.en ? `<tr><td><strong>Electronegativity</strong></td><td>${element.en}</td></tr>` : ''}
          ${element.ar ? `<tr><td><strong>Atomic Radius</strong></td><td>${element.ar} pm</td></tr>` : ''}
          ${element.ie ? `<tr><td><strong>Ionization Energy</strong></td><td>${element.ie} kJ/mol</td></tr>` : ''}
          ${element.ox ? `<tr><td><strong>Oxidation States</strong></td><td>${element.ox}</td></tr>` : ''}
          ${element.year ? `<tr><td><strong>Discovery Year</strong></td><td>${element.year}</td></tr>` : ''}
          ${element.discoverer ? `<tr><td><strong>Discovered By</strong></td><td>${element.discoverer}</td></tr>` : ''}
          ${element.uses ? `<tr><td><strong>Uses &amp; Applications</strong></td><td>${element.uses}</td></tr>` : ''}
        </table>
        <div class="footer">
          <p>Generated by CHEMNEST DICTIONARY - ${new Date().toLocaleDateString()}</p>
        </div>
      </body>
    </html>
  `);
...
function exportToImage(elementId, filename) {
  // Note: This requires html2canvas library in production
  // For now, we provide a download button
  alert('Image export would require html2canvas library. Use PDF export or screenshot instead.');
}
...
// Search and Filter
// Advanced Global Search with Fuzzy Matching
...
function performGlobalSearch(query) {
  ...
  const elementResults = chemistryData.elements.filter(el => 
    ...
    el.num.toString().includes(query)
  );
  
  const compoundResults = [
    ...chemistryData.compounds.inorganic.filter(c => 
      c.name.toLowerCase().includes(query) || c.formula.toLowerCase().includes(query)
    ).slice(0, 10),
    ...chemistryData.compounds.organic.filter(c => 
      c.name.toLowerCase().includes(query) || c.formula.toLowerCase().includes(query)
    ).slice(0, 10)
  ];
  
  const dictionaryResults = chemistryData.dictionaryTerms.filter(t => 
    t.term.toLowerCase().includes(query) || 
    t.definition.toLowerCase().includes(query)
  ).slice(0, 10);
  
  // Log results for debugging
  console.log(`Global search results for "${query}":`);
  console.log(`- Elements: ${elementResults.length}`);
  console.log(`- Compounds: ${compoundResults.length}`);
  console.log(`- Dictionary: ${dictionaryResults.length}`);
...
function setupSearch() {
  ...
  const debouncedFilterElements = debounce((query, category) => {
    filterElements(query, category);
  }, 300);
  
  const debouncedFilterCompounds = debounce((query) => {
    filterCompounds(query);
  }, 300);
  
  const debouncedFilterElementsList = debounce((query, category) => {
    filterElementsList(query, category);
  }, 300);
  
  elementSearch?.addEventListener('input', (e) => {
    const query = e.target.value.toLowerCase();
    debouncedFilterElements(query, categoryFilter.value);
  });
  ...
  compoundSearch?.addEventListener('input', (e) => {
    const query = e.target.value.toLowerCase();
    debouncedFilterCompounds(query);
  });
  
  elementsListSearch?.addEventListener('input', (e) => {
    const query = e.target.value.toLowerCase();
    debouncedFilterElementsList(query, elementsListCategory.value);
  });
...
// Render Elements List - OPTIMIZED with Virtual Scrolling
function renderElementsList() {
  ...
  if (chemistryData.elements.length > 50) {
    ...
    // For smaller lists, render normally with fragment
    const fragment = document.createDocumentFragment();
...
function renderReactions() {
  ...
  if (!inorganicContainer || !organicContainer) return;
  
  // Use DocumentFragment for efficient rendering
  const inorganicFragment = document.createDocumentFragment();
  const organicFragment = document.createDocumentFragment();
  
  chemistryData.reactions.inorganic.forEach(r => {
    ...
    `;
    inorganicFragment.appendChild(div);
  });
  
  chemistryData.reactions.organic.forEach(r => {
    const div = document.createElement('div');
    div.className = 'reaction-item';
    div.innerHTML = `
      <div class="reaction-type">${r.type}</div>
      ...
      <div class="reaction-example"><strong>Mechanism:</strong> ${r.mechanism}</div>
    `;
    organicFragment.appendChild(div);
  });
  
  inorganicContainer.innerHTML = '';
  organicContainer.innerHTML = '';
  inorganicContainer.appendChild(inorganicFragment);
  organicContainer.appendChild(organicFragment);
  
  console.log(`✅ Rendered ${chemistryData.reactions.inorganic.length} inorganic + ${chemistryData.reactions.organic.length} organic reactions`);
}
